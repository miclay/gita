...
cmd.setGitConfig=function(key,value){
	if(key===undefined||value===undefined) return;
	cmd.exec('git config '+key+' '+value);
};
...
cmd.getGitDefaultBranch=function(){
	var result=cmd.execSync('git config branch.default')||{};
	if(!result.stderr){
		return utils.trim(result.stdout||'master');
	}else{
		return '';
	}
};
